import { MessageParseError, ConstellationError, CancelledError, EventTimeoutError } from './errors';
    ...
    public send(packet: Packet): Promise<any> {
		...
		const promise = new Promise((resolve, reject) => {
			...
			timer = setTimeout(()=> {
				this.removeListener(`reply:${packet.id()}`, onReply);
				this.removeListener("close", onClose);
				reject(new EventTimeoutError(`${packet.id()}`));
			}, timeout);
		});
    ...
    private schedulePing() {
        ...
        this.pingTimeout = setTimeout(() => {
			...
			onClose = () => {
				...
				this.removeListener(`reply:${packet.id()}`, onReply);
				this.socket.close();
				this.emit('warning', new CancelledError());
			};
			this.once(`reply:${packet.id()}`, onReply);
			this.once("close", onClose);
			...
			timer = setTimeout(() => {
				...
				this.removeListener("close", onClose);
				this.socket.close();
				this.emit('warning', new EventTimeoutError("ping"));
			}, timeout);
